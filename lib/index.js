(function () {
	...
	var initializeAdminModel = function(app)
	{
		...
		acre.pre(acre.CREATE, acre.options.rootPath + '/acreadmins', function(request, response, next){
			...
			{
				request.body.password = crypto.createHmac('sha256', acre.options.adminPasswordHashSecret).update(request.body.password).digest('hex');
			}
			next();
		});
				...
				else
				{
					...
					if (_.isArray(admins))
					{
						...
						for (i in admins)
						{
							...
							{
								if (admins[i].password === crypto.createHmac('sha256', acre.options.adminPasswordHashSecret).update(request.body.password).digest('hex'))
								{
    ...
    acre.options = {
		...
		adminRoute: '/admin',
		appName: 'Acre App',
		adminPasswordHashSecret: '87fc9f12e96ad14e1899ff5e429d5b9066cb3aecb80bc62d52967662590dacd7'
    };
