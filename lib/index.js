(function () {
	...
	var create = function(Model, request, response, callback)
	{
		...
		var isPrivate = getUserCallback(acre.CREATE, acre.PRIVATE, path);

		if (!hasAdminSession() && isPrivate)
		{
			respondError(response, 401, "Unauthorized");
		}
		else if (isModelRootPath(Model, path))
		{
	...
	var retrieve = function(Model, request, response, callback)
	{
		...
		var isPrivate = getUserCallback(acre.RETRIEVE, acre.PRIVATE, path);
		var handleFind, hndl;
	...
	var update = function(Model, request, response, callback)
	{
		...
		var isPrivate = getUserCallback(acre.UPDATE, acre.PRIVATE, path);
	...
	var remove = function(Model, request, response, callback)
	{
		...
		var isPrivate = getUserCallback(acre.REMOVE, acre.PRIVATE, path);
