(function () {
    ...
    {
		var createOverride = getUserCallback(acre.CREATE, acre.OVERRIDE, path);
		var retrieveOverride = getUserCallback(acre.RETRIEVE, acre.OVERRIDE, path);
		var updateOverride = getUserCallback(acre.UPDATE, acre.OVERRIDE, path);
		var deleteOverride = getUserCallback(acre.REMOVE, acre.OVERRIDE, path);

		// Create
		if (createOverride)
		{
			console.log("setting " + createVerb.toUpperCase() + " URI for " + path);
			app[createVerb](path, getUserCallback(acre.CREATE, acre.PRE, path), function(request, response){
				create(model, request, response, getUserCallback(acre.CREATE, acre.POST, path));
			});
		}
		else
		{
		...
		if (!retrieveOverride)
		{
			...
			var callbacks = getUserCallback(acre.RETRIEVE, acre.PRE, path);
			app.get(path, callbacks, function(request, response){
				retrieve(model, request, response, getUserCallback(acre.RETRIEVE, acre.POST, path));
			});
		}
		else
		{
		...
		if (!updateOverride)
		{
			...
			app[updateVerb](path, getUserCallback(acre.UPDATE, acre.PRE, path), function(request, response){
				update(model, request, response, getUserCallback(acre.UPDATE, acre.POST, path));
			});
		}
		else
		{
		...
		if (!deleteOverride)
		{
			...
			app["delete"](path, getUserCallback(acre.REMOVE, acre.PRE, path), function(request, response){
				erase(model, request, response, getUserCallback(acre.REMOVE, acre.POST, path));
			});
		}
		else
		{
