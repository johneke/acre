(function () {
    ...
    var setRESTPaths = function (app, path, Model)
    {
		...
		if (!acre.routes.hasOwnProperty(Model.modelName))
		{
		...
		{
			if (!acre.routes[Model.modelName].hasOwnProperty('routes'))
			{
	...
	var bindAdminRoutes = function(app, models){
		...
		for (i in models)
		{
			var retrieveRoutePath = acre.options.adminRoute + "/" + models[i].modelName+ "-list.html";
			var updateRoutePath = acre.options.adminRoute + "/" + models[i].modelName+ "-detail.html";
			var model = {
				name: models[i].modelName,
				route: acre.routes[models[i].modelName].rootResource,
				module: {
					name: models[i].modelName,
					resource: models[i].modelName + "-resource"
				},
				collection: models[i].collection.name,
				controllers: {
					create: models[i].modelName + "CreateController",
					retrieve: models[i].modelName + "RetrieveController",
					update: models[i].modelName + "UpdateController"
				},
				views: {
					create: updateRoutePath,
			...
			//compile data for sub models
			var routes = acre.routes[models[i].modelName].routes;
			for (j in routes)
			{
				var identifiers, isArray, route = routes[j], name;

				var frags = route.split('/');
				frags.shift(); //shift empty string

				//get name of the sub model
				//name = frags[frags.length - 1].indexOf(':') === 0 ? frags[frags.length - 2] : frags[frags.length - 1];
				name = '_'+randomString(7);

				//set whether or not this sub model is an array
				isArray = frags[frags.length - 1].indexOf(':') === 0;

				//get all ids that this nested model depends on
				frags = frags.filter(idFilter);
				for (k = 0; k < frags.length; k++)
				{
					frags[k] = frags[k].replace(':', '');
				}

				identifiers = '{';
				for (k = 0; k < frags.length; k++)
				{
					identifiers += frags[k] + ': this.' + frags[k];
					if (k !== frags.length - 1)
					{
						identifiers += ',';
					}
			...
			viewPath = __dirname + '/portal/views/admin-model-list-view.ejs';
			app.get(retrieveRoutePath, getRoute(viewPath, 'text/html', {model: model}));

			viewPath = __dirname + '/portal/views/admin-model-detail-view.ejs';
			app.get(updateRoutePath, getRoute(viewPath, 'text/html', {model: model}));

			routePath = acre.options.adminRoute + "/" + models[i].modelName+ "-controllers.js";
			viewPath = __dirname + '/portal/views/admin-model-controllers.ejs';
			app.get(routePath, getRoute(viewPath, 'text/javascript', {model: model}));
			jsPaths.push(routePath);

			routePath = acre.options.adminRoute + "/" + models[i].modelName+ "-resource.js";
			viewPath = __dirname + '/portal/views/admin-model-resource.ejs';
			app.get(routePath, getRoute(viewPath, 'text/javascript', {model: model, createVerb: createVerb.toUpperCase(), updateVerb: updateVerb.toUpperCase()}));
			jsPaths.push(routePath);
		...
		app.get(routePath, getRoute(viewPath, 'text/html', {models: _models, js: jsPaths}));
	};
