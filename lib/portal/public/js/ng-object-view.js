		...
		var linker = function(scope, element, attrs) {
			...
			function getTemplate(field, model, isArrayItem, arrayModel, instance, path)
			{
				...
				if (_.isPlainObject(field))
				{
					...
					if (isArrayItem)
					{
						content += '<span class="acre-ov-container-title">{legend}<button class="btn btn-small btn-danger acre-array-action-button" data-ng-click="pop(\'{path}\')" data-ng-disabled="{model}.length < 2">Remove</button></span><br />'.replace(/{model}/gi, arrayModel);
					}
					else
					{
